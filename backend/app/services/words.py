...
from ..schemas.words import WordResponse, DictionaryData, FDAData
...
"""
async def generate_enhanced_word_info(dictionary_data: DictionaryData, free_dictionary_data: Optional[FDAData] = None) -> WordResponse:
    ...
    - `example_sentences`: `raw_examples`を参考に、英語と日本語の両方を含む新しい例文を最大5つ生成してください。英語の例文は、{word}を確実に含む自然かつ簡潔な文である必要があります。日本語の例文は、英語の例文を自然な「日本語」に翻訳してください。例文は、与えられた単語の意味を明確に示す、純粋な日本語のものでなければなりません。例文は、`english`と`japanese`のペアで表現してください。
    ...
    try:
        ...
        word_info = WordResponse.model_validate_json(json_str)
