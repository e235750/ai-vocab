...
export default function AddCardForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (definitions[0]?.part_of_speech.trim() === '') {
      definitions[0].part_of_speech = '未分類'
    }
    const cleanedDefinitions = definitions.filter(
      (d) =>
        d.part_of_speech.trim() !== '' &&
        d.japanese.some((j) => j.trim() !== '')
    )
    const newCardData = {
      english,
